...
ssl_stunel(){
    ...
    if [[ ! "$(echo "$db"|grep ".crt")" = "" ]]; then
        ...
        read opcion
        if [[ $opcion != @(n|N) ]]; then
            cp ${XCLAN_crt}/$cert ${XCLAN_tmp}/stunnel.crt
            cp ${XCLAN_crt}/$key ${XCLAN_tmp}/stunnel.key
        fi
    fi

    if [[ $opcion != @(s|S) ]]; then
        openssl genrsa -out ${XCLAN_tmp}/stunnel.key 2048 > /dev/null 2>&1
        (echo "" ; echo "" ; echo "" ; echo "" ; echo "" ; echo "" ; echo "@cloudflare" )|openssl req -new -key ${XCLAN_tmp}/stunnel.key -x509 -days 1000 -out ${XCLAN_tmp}/stunnel.crt > /dev/null 2>&1
    fi
    cat ${XCLAN_tmp}/stunnel.key ${XCLAN_tmp}/stunnel.crt > /etc/stunnel/stunnel.pem
    ...
    msg -bar
    rm -rf ${XCLAN_tmp}/stunnel.crt > /dev/null 2>&1
    rm -rf ${XCLAN_tmp}/stunnel.key > /dev/null 2>&1
    sleep 3
    return
}
