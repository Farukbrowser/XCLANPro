...
updateProject(){
    ...
    bash <(curl -L -s https://raw.githubusercontent.com/Farukbrowser/XCLANPro/main/Utils/v2ray/go.sh) --version v4.45.2
}
...
installFinish(){
    ...
    jq '.inbounds[].streamSettings += {"network":"ws","wsSettings":{"path": "/XCLANPro/","headers": {"Host": "ejemplo.com"}}}' < /etc/v2ray/tmp.json >> /etc/v2ray/config.json
